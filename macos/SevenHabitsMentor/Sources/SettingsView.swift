...
struct SettingsView: View {
  ...
  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 18) {
        ...
        MentorPanel {
          ...
          settingsBlock(title: "导师服务") {
            VStack(alignment: .leading, spacing: 12) {
              ...
              SecureField("Qoder PAT（可选，润色话术）", text: $model.qoderPat)
                ...
                .padding(10)
                .background(MentorTheme.paper.opacity(0.8))
                .overlay(
              ...
              Text("填 PAT 可用云端润色；不填也能对话。决策逻辑在内置运行时。")
                .font(.caption)
                .foregroundStyle(MentorTheme.muted)
              ...
              DisclosureGroup("开发者选项") {
                ...
                Text("发行版带 MentorAgent.tgz。此处仅给源码开发回退。")
                  .font(.caption)
                  .foregroundStyle(MentorTheme.muted)
              }
          ...
          settingsBlock(title: "日历与提醒") {
            VStack(alignment: .leading, spacing: 10) {
              metric("日历授权", model.calendarAuthorized ? "已授权" : "未授权")
              Text("寄生系统日历：读取近 4 周事件，大石头写回 EventKit。")
                .font(.caption)
                .foregroundStyle(MentorTheme.muted)
              HStack(spacing: 8) {
          ...
          settingsBlock(title: "演示") {
            VStack(alignment: .leading, spacing: 8) {
              ...
              if model.missedWeeklyReviews > 0 {
                Text("已跳过周回顾 \(model.missedWeeklyReviews) 次")
                  .font(.caption)
                  .foregroundStyle(MentorTheme.muted)
              }
            }
    ...
    .background(
      ZStack {
        MentorTheme.paper
        RadialGradient(
          colors: [MentorTheme.accent.opacity(0.07), .clear],
          center: .topTrailing,
          startRadius: 40,
          endRadius: 480
        )
  ...
  private func settingsBlock<Content: View>(
    ...
    VStack(alignment: .leading, spacing: 12) {
      MentorSectionHeader(title: title)
      content()
    }
    .padding(18)
  }

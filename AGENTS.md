...
- **Conversation architecture (REQUIREMENTS §8)**: Stage B is implemented. Per-turn flow is `understand` → `respond(understanding)` → optional `phrase`. Structured schema lives in `src/types/understanding.ts`; local degrade is `understandLocal` in `src/services/understanding.ts`. Model understanding runs on the agent server when PAT is available (`/api/mentor/understand` + wired into `/api/mentor/turn`). Stage C (bounded actions / stay_and_probe) is not open yet.

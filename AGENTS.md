...
- **Conversation architecture (REQUIREMENTS §8)**: Stage B + Stage C are implemented. Per-turn flow is `understand` → `propose(action)` → `referee` → `respond(understanding, decision)` → optional `phrase`. Structured understanding lives in `src/types/understanding.ts`; bounded actions + referee in `src/types/actions.ts` / `src/services/actions.ts`. Local degrade: keyword understanding + advance-biased proposals (no probe). Model paths: `/api/mentor/understand`, `/api/mentor/action`, wired into `/api/mentor/turn`.
